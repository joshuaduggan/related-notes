...
// debug and local are separate so I can break them apart later
define('DEBUG', true);
//define('DEBUG', false);
define('LOCAL', DEBUG);
define('VERSION', '0.10' . (LOCAL ? '-loc' : '-net'));

# Requires a php ini file like this:
# username = jsmith
# password = secret
define('DB_LOGIN_INI_FILE', '../../misc_dev_db_login.ini');
$db = null;
{
  $dbLogin = parse_ini_file(DB_LOGIN_INI_FILE);
  $db = new mysqli(
      (LOCAL ? 'localhost' : 'mysql.joshuaduggan.com'),
      $dbLogin['username'],
      $dbLogin['password'],
      'relatednotes');
  unset($dbLogin);
...
function authenticateUser($pass) {
  ...
  if (!($dbUserPassHash = getSessionUserPassHash())) return false;
